...
export default function LoginPage() {
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 sm:p-6">
      {/* Botón Volver - Más arriba en móviles */}
      <Link 
        href="/" 
      ...
      <div className="w-full max-w-[400px] bg-zinc-900/50 border border-pink-500/20 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <form className="space-y-4 sm:space-y-6" onSubmit={(e) => e.preventDefault()}>
          <div className="space-y-1">
            <label className="text-[9px] sm:text-[10px] uppercase tracking-widest text-zinc-400 ml-4">Usuario</label>
            <div className="relative">
          ...
          <button className="w-full bg-gradient-to-r from-pink-500 to-rose-600 text-white font-bold py-3.5 sm:py-4 rounded-full shadow-lg hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 mt-2">
        ...
        <div className="mt-6 sm:mt-8 text-center">
          <p className="text-zinc-500 text-[11px] sm:text-xs">
            ¿No tienes cuenta?{" "}
            <Link href="/admin/users/new" className="text-pink-400 hover:underline font-medium">
              Regístrate aquí
            </Link>
          </p>
        </div>
