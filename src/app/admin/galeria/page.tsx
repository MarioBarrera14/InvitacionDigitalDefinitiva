...
import { ImagePlus, Layout, Upload, Film, Play, X, CheckCircle2, Loader2, Eraser, Music, Headset, FileAudio } from "lucide-react";
...
export default function GestionGaleria() {
  ...
  const handleMusicFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMusicFile(url);
    } catch (err) { Swal.fire("Error", "No se pudo subir la música", "error"); } finally { setLoading(null); }
  };
  ...
  const handleMainFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (err) { setLoading(null); } finally { setLoading(null); }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 p-4 md:p-8">
      ...
      {/* HEADER ELEGANTE CON BOTÓN LIMPIAR */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-zinc-100 pb-10">
        ...
        <div className="flex items-center gap-3">
          {/* BOTÓN LIMPIAR TODO RECUPERADO */}
          <button 
            onClick={handleLimpiarTodo} 
            className="group flex items-center gap-2 px-5 py-5 rounded-[1.5rem] border border-zinc-200 text-zinc-400 hover:text-rose-500 hover:border-rose-100 hover:bg-rose-50 transition-all active:scale-95"
          >
            <Eraser size={18} />
          ...
          <button 
            ...
            disabled={isSaving} 
            className="bg-zinc-900 hover:bg-indigo-600 text-white px-10 py-5 rounded-[1.5rem] text-[11px] font-bold tracking-[0.2em] flex items-center gap-3 shadow-2xl transition-all active:scale-95 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-4 space-y-8">
          <section className="bg-white p-8 rounded-[3rem] border border-zinc-100 shadow-xl shadow-zinc-100/50">
            ...
            <div className="relative aspect-[3/4] rounded-[2rem] overflow-hidden group">
              <div onClick={() => mainInputRef.current?.click()} className="w-full h-full bg-zinc-50 border-2 border-dashed border-zinc-200 flex items-center justify-center cursor-pointer hover:bg-zinc-100 transition-all">
                {loading === "main" ? <Loader2 className="animate-spin text-indigo-500" /> : fotoPrincipal ? <img src={fotoPrincipal} className="w-full h-full object-cover" /> : <ImagePlus className="text-zinc-300" size={40} />}
              </div>
          ...
          <section className="bg-zinc-900 p-8 rounded-[3rem] text-white shadow-2xl">
            ...
            <div 
              ...
              className="group relative p-6 bg-white/5 border border-white/10 rounded-3xl cursor-pointer hover:bg-white/10 transition-all border-dashed"
            >
              {loading === "music" ? (
                <div className="flex flex-col items-center gap-2 py-4"><Loader2 className="animate-spin text-indigo-400" /><p className="text-[10px] text-zinc-500 uppercase">Subiendo audio...</p></div>
              ) : (
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded-2xl ${musicFile ? 'bg-indigo-500 text-white' : 'bg-zinc-800 text-zinc-500'}`}>
                    <FileAudio size={24} className={musicFile ? "animate-pulse" : ""} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Archivo MP3</p>
            ...
            <input type="file" ref={musicInputRef} className="hidden" accept="audio/*" onChange={handleMusicFile} />
          </section>
        </div>
        ...
        <div className="md:col-span-8 space-y-8">
          <section className="bg-white p-8 rounded-[3rem] border border-zinc-100 shadow-xl shadow-zinc-100/50">
            <h2 className="text-[12px] font-black uppercase tracking-tighter text-zinc-400 mb-8 flex items-center gap-2">03. Video Destacado (Grande)</h2>
            <div className="relative aspect-video rounded-[2.5rem] overflow-hidden bg-zinc-900">
              <div 
                onClick={() => videoInputRef.current?.click()} 
                className="w-full h-full flex items-center justify-center cursor-pointer group"
              >
                {loading === "video-main" ? <Loader2 className="animate-spin text-indigo-400" size={40} /> : videoFile ? <video src={videoFile} className="w-full h-full object-cover" autoPlay muted loop /> : <div className="text-center group-hover:scale-110 transition-transform"><div className="w-20 h-20 bg-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4 shadow-xl text-white"><Play fill="currentColor" size={32} /></div><p className="text-[11px] font-black text-zinc-500 uppercase tracking-widest">Click para subir video</p></div>}
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {carrusel.map((url, index) => (
                <div key={index} className="relative aspect-square">
                  <div onClick={() => carruselRefs.current[index]?.click()} className={`w-full h-full rounded-3xl overflow-hidden border-2 transition-all flex items-center justify-center cursor-pointer ${url ? 'border-white shadow-lg' : 'border-dashed border-zinc-200 bg-white hover:border-indigo-300'}`}>
                    {loading === `carrusel-${index}` ? <Loader2 className="animate-spin text-indigo-500" size={20} /> : url ? (isVideo(url) ? <video src={url} className="w-full h-full object-cover" muted loop autoPlay /> : <img src={url} className="w-full h-full object-cover" />) : <Upload size={20} className="text-zinc-300" />}
                  </div>
                  {url && <button onClick={(e) => { e.stopPropagation(); const n = [...carrusel]; n[index]=null; setCarrusel(n); }} className="absolute -top-2 -right-2 p-2 bg-white rounded-full text-rose-500 shadow-md hover:bg-rose-500 hover:text-white transition-all"><X size={12}/></button>}
