...
// --- COMPONENTE MODAL (Estilo Boutique) ---
...
// --- BOTÓN DE COPIAR (Estilo Boutique) ---
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="flex flex-col items-center gap-1 group w-full py-3 border border-black/5 rounded-2xl hover:bg-black/5 transition-all">
      <span className="text-[10px] text-black/40 uppercase tracking-widest font-medium">Toca para copiar</span>
...
export function Details() {
  const { dressCode, regalo } = eventConfig;
  ...
  return (
    <section className="relative bg-black pt-32 pb-48 md:pt-48 md:pb-64 overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-20 text-center">
        ...
        {/* BOTONES CIRCULARES (Los iconos que te gustaron) */}
      ...
      {/* ONDA INFERIOR (La que ya tenías) */}
      ...
      {/* FONDO DE ONDAS ANIMADAS (El que ya tenías) */}
      ...
      {/* --- MODALES --- */}
      <DetailModal isOpen={activeModal === "dress"} onClose={() => setActiveModal(null)} title={dressCode.titulo}>
        <p className="text-lg">Para esta noche mágica, el estilo es <br />
          <strong className="text-black font-semibold uppercase tracking-widest text-xl block mt-2 italic">Elegante Sport</strong>
        </p>
      </DetailModal>

      <DetailModal isOpen={activeModal === "gift"} onClose={() => setActiveModal(null)} title={regalo.titulo}>
        <p className="mb-8">Tu presencia es mi mayor alegría. Pero si deseas hacerme un presente, podés usar estos datos:</p>
        <div className="space-y-3">
          <div className="bg-black/5 p-4 rounded-2xl">
            <p className="text-[10px] uppercase tracking-[0.2em] text-black/40 mb-1">Titular</p>
            <p className="text-black font-semibold text-lg">{regalo.datosBancarios.titular}</p>
          </div>
          <CopyButton text={regalo.datosBancarios.cbu} />
          <CopyButton text={regalo.datosBancarios.alias} />
          <p className="text-xs italic text-black/40 mt-4 tracking-widest uppercase">{regalo.datosBancarios.banco}</p>
        </div>
      </DetailModal>
